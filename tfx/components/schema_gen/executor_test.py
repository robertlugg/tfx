...
class ExecutorTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self.fixed_schema = standard_artifacts.Schema()
    self.fixed_schema.uri = os.path.join(self.source_data_dir, 'fixed_schema/')
  ...
  def testDoWithoutFixedSchema(self):
  ...
  def testDoWithFixedSchema(self):
    self.input_dict['fixed_schema'] = [self.fixed_schema]
    schema_gen_executor = executor.Executor()
    schema_gen_executor.Do(self.input_dict, self.output_dict,
                           self.exec_properties)
    self.assertNotEqual(0, len(tf.gfile.ListDirectory(self.schema_output.uri)))
    self._assertSchemaEqual(self.fixed_schema, self.schema_output)

  def testDoWithNonExistentFixedSchema(self):
    non_existent_schema = standard_artifacts.Schema()
    non_existent_schema.uri = '/path/to/non_existent/schema'

    self.input_dict['fixed_schema'] = [non_existent_schema]
    schema_gen_executor = executor.Executor()
    schema_gen_executor.Do(self.input_dict, self.output_dict,
                           self.exec_properties)
    self.assertNotEqual(0, len(tf.gfile.ListDirectory(self.schema_output.uri)))
  ...
  def testDoWithNonExistentFixedSchemaWithoutStats(self):
    non_existent_schema = standard_artifacts.Schema()
    non_existent_schema.uri = '/path/to/non_existent/schema'
