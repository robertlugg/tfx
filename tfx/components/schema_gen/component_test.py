...
class SchemaGenTest(tf.test.TestCase):
  ...
  def testConstructWithFixedSchema(self):
    schema_gen = component.SchemaGen(
        fixed_schema=channel_utils.as_channel([standard_artifacts.Schema()]))
    self.assertEqual('SchemaPath', schema_gen.outputs.output.type_name)

  def testConstructWithNeitherStatsNorFixedSchema(self):
